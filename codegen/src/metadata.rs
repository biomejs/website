...
impl RegistryVisitor<HtmlLanguage> for Metadata {
    ...
    {
        self.register_rule::<R, HtmlLanguage>();
    }
}
...
struct RuleNames {
    lints: BTreeMap<&'static str, BTreeSet<&'static str>>,
    actions: BTreeMap<&'static str, BTreeSet<&'static str>>,
}
...
impl RuleNames {
    ...
    where
        ...
        R: Rule<Query: Queryable<Language = L, Output: Clone>> + 'static,
    {
        let names = match <R::Group as RuleGroup>::Category::CATEGORY {
            RuleCategory::Lint => &mut self.lints,
            RuleCategory::Action => &mut self.actions,
            _ => return,
        };
        names
            .entry(R::Group::NAME)
...
impl RegistryVisitor<HtmlLanguage> for RuleNames {
    fn record_rule<R>(&mut self)
    where
        R: Rule<Query: Queryable<Language = HtmlLanguage, Output: Clone>> + 'static,
    {
...
pub fn generate_playground_rules() -> anyhow::Result<()> {
    ...
    for (filename, constant, names) in [
        ("lintRules.ts", "LINT_RULES", visitor.lints),
        ("assistActions.ts", "ASSIST_ACTIONS", visitor.actions),
    ] {
        let rules_file = project_root()
            .join("src/playground/generated")
            .join(filename);
        let content = format!(
