...
pub fn generate_env_variables()-> Result<()> {
    let file_path =
        project_root().join("src/content/docs/reference/environment_variables.md");
    ...
    writeln!(content, "### `{}`\n\n {}\n", env.biome_log_prefix.name(), env.biome_log_prefix.description() )?;
    writeln!(content, "### `{}`\n\n {}\n", env.biome_log_path.name(), env.biome_log_path.description() )?;
    writeln!(content, "### `{}`\n\n {}\n", env.biome_config_path.name(), env.biome_config_path.description() )?;
