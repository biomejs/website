...
Previews configuration updates required by breaking changes. The `prettier`, `eslint`, and `stylelint` subcommands instead import settings from those tools.
...
Imports an Stylelint configuration and ignore settings from the current working directory into the Biome configuration.

The `stylelint` subcommand accepts these additional flags:
