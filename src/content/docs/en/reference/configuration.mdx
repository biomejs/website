...
A list of paths to other Biome configuration files or npm packages that export a configuration file. Biome resolves and applies
...
{
  "extends": [
    "@org/base", // this is an npm package that exports a configuration
    "./configs/format.json" // this is a local file
  ]
}
```

Check how to share your configuration as an npm package in the relative [guide](/guides/big-projects#exporting-a-biome-configuration-from-an-npm-package).

This option also accepts a string that must match the value `"//"`, which can be used when setting up [monorepos](/guides/big-projects#monorepo).
...
A list of [GritQL plugins](/linter/plugins/) to enable. Each entry can be either a simple path string or an object with a path and optional settings.
...
Resolving a plugin from the configuration file that declares it:

```json title="biome.shared.jsonc"
{
  "plugins": [
    {
      "path": "./plugins/no-object-assign.grit",
...
You can mix string and object entries in the same `plugins` list.
...
### `plugins.<ITEM>.resolutionKind`

Controls how the plugin's `path` is resolved:

- `"project"` resolves relative plugin paths from the consuming project. This is the default behaviour and matches how other relative paths in shared configuration files are resolved.

- `"config"` resolves relative plugin paths from the configuration file that declared the plugin. This is useful for shared configurations loaded through `extends` that need to load plugins shipped next to the shared configuration.

This option does not change how `includes` globs are interpreted.
...
Enables the parsing of frontmatter in Markdown files. When enabled, the leading `---` is considered the opening fence of the frontmatter.
...
Enables the parsing of GitHub Flavored Markdown (GFM). When enabled, the following extensions are parsed:
- task lists items
- strikethrough inline markup
- tables

```md
- [ ] This is a disabled task
- [x] This is an enabled task
...
### `markdown.formatter.enabled`

Enables Biome's formatter for Markdown files.

> Default: `true`

### `markdown.formatter.indentStyle`

The style of the indentation for Markdown files. It can be `"tab"` or `"space"`.

> Default: `"tab"`

### `markdown.formatter.indentWidth`

How big the indentation should be for Markdown files.

> Default: `2`

### `markdown.formatter.lineEnding`
...
The amount of characters that can be written on a single line in Markdown files.

> Default: `80`

### `markdown.formatter.proseWrap`

Controls how paragraphs should be formatted. The option accepts the following values:

- `"preserve"`: Biome won't attempt to format any paragraph, and it will the newlines of the user. This is the **default** value.
- `"always"`: Biome attempts to fit the paragraph in the configured `lineWidth`
  ...
  Lorem ipsum dolor sit amet, consectetur adipiscing elit.
  ```
  If you configure the `lineWidth` to a value of `20`, Biome will format the paragraphs to that the words will fit the constraint:
  ...
  consectetur adipiscing elit.
  ```
- `"never"`: Biome will format each paragraph to a single line, regardless of the configured `lineWidth`.

  The following paragraph will format on a single line.
  ```diff title="file.md"
  -Lorem ipsum dolor sit amet,
  -consectetur adipiscing elit.
  +Lorem ipsum dolor sit amet, consectetur adipiscing elit.
...
Enables Biome's linter for Markdown files.
...
### `yaml.formatter.enabled`

Enables Biome's formatter for YAML files.
...
How big the indentation should be for YAML files.

> Default: `2`

### `yaml.formatter.lineEnding`

The type of line ending for YAML files.
- `"lf"`, Line Feed only (`\n`), common on Linux and macOS as well as inside git repos;
- `"crlf"`, Carriage Return + Line Feed characters (`\r\n`), common on Windows;
- `"cr"`, Carriage Return character only (`\r`), used very rarely.
...
### `yaml.formatter.lineWidth`

The amount of characters that can be written on a single line in YAML files.
