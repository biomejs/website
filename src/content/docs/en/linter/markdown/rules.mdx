...
description: List of available lint rules for Markdown
localized: false
---
...
| --- | --- | --- |
| [useConsistentHeadingLevel](/linter/rules/use-consistent-heading-level/markdown) | Enforce that all heading levels are consistent and ordered. |  |
| [useFencedCodeLanguage](/linter/rules/use-fenced-code-language/markdown) | Enforce that fenced code blocks specify a code tag (language). | <span class='inline-icon' title="This rule is not released yet"><Icon name="moon" label="This rule is not released yet" size="1.2rem" /></span> |
| [useSingleTopLevelHeading](/linter/rules/use-single-top-level-heading/markdown) | Enforce that a Markdown document has a single top-level heading. |  |
| [useTopLevelHeading](/linter/rules/use-top-level-heading/markdown) | Require Markdown documents to start with a top-level heading. |  |
## Recommended rules 
