...
| [useSingleTopLevelHeading](/linter/rules/use-single-top-level-heading/markdown) | Enforce that a Markdown document has a single top-level heading. | <span class='inline-icon' title="This rule is not released yet"><Icon name="moon" label="This rule is not released yet" size="1.2rem" /></span> |
