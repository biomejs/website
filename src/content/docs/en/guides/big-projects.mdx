...
When using `extends`, relative paths from a shared configuration are
interpreted from the extending configuration, not from the folder of the
shared file itself. Plugin paths normally follow that same rule too.

If a shared configuration ships with its own plugin, see
[Resolving Plugins from Shared Configurations](/linter/plugins/#resolving-plugins-from-shared-configurations).
...
If you need to share your configuration file across multiple projects, you can do so by [publishing it as npm package](/guides/publish-packages).
