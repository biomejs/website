---
title: Manual installation
description: Install the Biome manually
---

import { Code } from '@astrojs/starlight/components';
import { getLatestVersion } from "@biomejs/version-utils";
export const version = await getLatestVersion("stable");

Using Biome's standalone CLI binary can be a great choice if you aren't already using Node.js or `npm` (or any other package manager).
Or in other words, Biome shouldn't be the only reason for you to have a `package.json`.

:::note
If you're already using `npm` or another package manager, then using the package manager is the [preferred way to install](/guides/getting-started#installation) Biome.
You're already familiar with the tooling, and installing and updating are simpler.
:::

## Supported platforms

You have to pick the correct binary for your platform for Biome work. The following table should help you do so.

| Platform     | `arm64`                      | `x64`              |
| ------------ | ---------------------------- | ------------------ |
| Windows      | `win32-arm64`                | `win32-x64`        |
| macOS        | `darwin-arm64` (M1 or newer) | `darwin-x64`       |
| Linux        | `linux-arm64`                | `linux-x64`        |
| Linux (musl) | `linux-arm64-musl`           | `linux-x64-musl`   |

:::note
Use the Linux variant for Windows Subsystem for Linux (WSL).
:::

## Homebrew

Biome is available as a [Homebrew formula](https://formulae.brew.sh/formula/biome) for macOS and Linux users.

```shell showLineNumbers=false
brew install biome
```

## Winget

Biome is available as a community maintained package from the [Winget repository](https://github.com/microsoft/winget-pkgs/tree/master/manifests/b/biomejs/biome) for Windows users.

```shell showLineNumbers=false
winget install biomejs.biome
```

## Arch Linux (x86_64) 

Biome is available as a community maintained binary from the [Arch Linux Extra repository](https://archlinux.org/packages/extra/x86_64/biome). 

:::note
The [extra] repository is usually enabled by default in /etc/pacman.conf. If it is disabled, uncomment the following lines:
::: 

```ini showLineNumbers=false
[extra]
Include = /etc/pacman.d/mirrorlist
```

To install Biome run:

```shell showLineNumbers=false
pacman -S biome
```

## Docker

Biome publishes [official Docker images](https://github.com/biomejs/docker/pkgs/container/biome) that support
the **amd64** and **arm64** architectures for all Biome versions starting from `v1.7.0`.

```text showLineNumbers=false
ghcr.io/biomejs/biome:{major}
ghcr.io/biomejs/biome:{major}.{minor}
ghcr.io/biomejs/biome:{major}.{minor}.{patch}
```

Here are a couple examples on how to use the Docker image:

:::note
The default workdir is `/code` in the Docker image.
:::

```shell showLineNumbers=false
# Lint files
docker run -v $(pwd):/code ghcr.io/biomejs/biome lint
docker run -v $(pwd):/code ghcr.io/biomejs/biome lint --write

# Format files
docker run -v $(pwd):/code ghcr.io/biomejs/biome format
docker run -v $(pwd):/code ghcr.io/biomejs/biome format --write
```



## Using a published binary

### Linux and macOS

The installer downloads the latest stable binary to `${XDG_BIN_HOME:-$HOME/.local/bin}` and adds it to your `PATH`:

```shell
curl -fsSL https://biomejs.dev/install | bash
```

:::tip
You might need to restart your shell after installation before you can run `biome`.
:::

To install a specific version, pass `--version`:

```shell
curl -fsSL https://biomejs.dev/install | bash -s -- --version 2.5.9
```

You can also set `BIOME_INSTALL_DIR` to override the installation directory or pass `--no-modify-path` to leave your shell configuration unchanged.
Pass `--force` to download and reinstall a version that is already installed.

### Windows

The PowerShell installer downloads the latest stable binary to `%LOCALAPPDATA%\Biome\bin` and adds it to your user and current process `PATH`:

```powershell
irm https://biomejs.dev/install.ps1 | iex
```

To install a specific version, set `BIOME_VERSION` first:

```powershell
$env:BIOME_VERSION = "2.5.9"
irm https://biomejs.dev/install.ps1 | iex
```

You can also set `BIOME_INSTALL_DIR` to override the installation directory or pass `-NoModifyPath` to leave your `PATH` unchanged.
Pass `-Force` to download and reinstall a version that is already installed.

### Manual download

To install Biome, grab the executable for your platform from the [latest CLI release](https://github.com/biomejs/biome/releases) on GitHub and give it execution permission.

<Code lang="shell" showLineNumbers={false} code={`
# macOS arm (M1 or newer)
curl -L https://github.com/biomejs/biome/releases/download/@biomejs/biome@${version}/biome-darwin-arm64 -o biome
chmod +x biome

# Linux (x86_64)
curl -L https://github.com/biomejs/biome/releases/download/@biomejs/biome@${version}/biome-linux-x64 -o biome
chmod +x biome

# Windows (x86_64, Powershell)
Invoke-WebRequest -Uri "https://github.com/biomejs/biome/releases/download/@biomejs/biome@${version}/biome-win32-x64.exe" -OutFile "biome.exe"
`} />

Now you can use Biome by simply running `./biome`.

:::tip
To **upgrade** Biome to the latest version, run `biome upgrade`
:::

## Next Steps

Follow our [Getting Started guide](/guides/getting-started/).
