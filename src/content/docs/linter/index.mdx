...
You can quickly try Biome linter via CLI. The following command runs the linter on all files from the root of your project:
...
In Biome, rules have a naming convention, rules that start with `use*` are meant to enforce/suggest something, rules that start with `no*` are meant to deny something. When a rule encounters a *violation* of its concept, it emits a diagnostic.
...
Each lint rule ships with a default [severity](/diagnostics#diagnostic-severity), you can lean more about it by reading the documentation of the rule.
...
In Biome, rules *can* work across programming languages, that's because some rules might cover broad concepts that are language agnostic. This is a new, different vision on how linting should function. Refer to the documentation of the single rule to learn more about this.

Unlike other linters, Biome doesn't provide any rules that check for code formatting, because [Biome formatter](/formatter/) is designed to be in charge of it.

Many rules provide a **code fix** that can be automatically applied. However, Biome distinguishes them in [**safe fixes**](#safe-fixes) and [**unsafe fixes**](#unsafe-fixes), which work differently.
...
From a LSP-compatible editor, you can apply safe fixes **on save** with code action `source.fixAll.biome`.
...
From a LSP-compatible editor, it's not possible to apply all unsafe fixes on save due to their nature. However, you can review the single code fix and choose to apply it.
...
The [`"error"`](/reference/diagnostics#error) severity is useful when you want to block, for example, the CI if there's a violation that belongs to a certain rule.
...
Additionally, you can control the severity of lint rules **at group level**. Doing so, it's possible to control the diagnostic severity of **all rules** that belong to a group.
...
As explained before, the rules might emit code fixes that are **safe** or **unsafe**. Biome allows to turn a safe to an unsafe fix and vice-versa, or you can turn off the code fix entirely.

It's possible to configure the entity of the code fix, using the option `fix`.
  ...
  "linter": {
    "rules": {
      ...
      "style": {
        "useConst": {
          "level": "warn",
          "fix": "unsafe" // the code fir for useConst is now unsafe
        },
        "useTemplate": {
          "level": "warn",
          "fix": "safe" // the code fix for useTemplate is now safe
        }
      }
    }
  }
...
Domains are a Biome feature that allows to have more control over the lint rules. Domains could be seen as "concepts", or another form of category, however they offer different features compared to groups.

A domain:
- Can enable particular rules when Biome detects certain dependencies in the `package.json` of a project.
- Can add particular global variables.
- Can contains a small set of recommended rules.

Biome linter will enable the rules that belong to a domain when it detects certain dependencies in the nearest `package.json`. For example, if the `mocha` dependency is detected, Biome will enable the **recommended rules** of the [`test`](/linter/domains#test) domain.
...
If a code fix doesn't fall under these three guidelines, it's possible that the team forgot to make the rule fix safe. Open an issue or send a PR!