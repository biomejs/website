...
The linter is organized into groups and rules. Every rule belongs to a specific group.
...
The `info` severity won't affect the CLI at all, even when `--error-on-warnings` is passed. The diagnostic message is blue.
...
- `none`: the rule won't the code fix;
