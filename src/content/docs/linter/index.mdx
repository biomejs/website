...
Usually, by positioning your cursor in the range of the diagnostic and typing a certain short-cut (it varies by editor), a tooltip will appear with the possible code actions.
...
For example, if you want to turn off the code action of the rule `noUnusedVariables`, you'll use the code `quickfix.biome.correctness.noUnusedVariables`:
