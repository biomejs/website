...
| [noValueAtRule](/linter/rules/no-value-at-rule) | Disallow use of `@value` rule in css modules. | <span class='inline-icon' title="This rule is not recommended" ><Icon name="information" size="1.2rem" label="This rule is not recommended" /></span> |
