...
## Resolving Plugins from Shared Configurations

By default, plugin paths from shared configurations are resolved from the
consuming project. To load a plugin that ships with the shared configuration,
resolve it from the shared configuration file instead:
...
Then extend the shared configuration:

```json title="biome.json"
{
    "extends": ["./biome.shared.jsonc"]
}
```

The plugin path is resolved relative to `biome.shared.jsonc`, while `includes` still match files in the consuming project.

See [`plugins.<ITEM>.resolutionKind`](/reference/configuration/#pluginsitemresolutionkind) for the full option reference.
