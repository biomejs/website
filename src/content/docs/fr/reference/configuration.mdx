...
Nous publions un fichier de schéma JSON pour le `biome.json`.
...
Si vous avez des problèmes avec la résolution du fichier physique, vous pouvez utiliser celui publié dans ce site&nbsp;:
...
Une liste de chemins vers d’autres fichiers JSON. Biome résoud et fait appliquer les options des fichiers contenus dans la liste `extends`, puis fait appliquer celles contenues dans le fichier `biome.json`.
...
cette limite seront ignorés pour des questions de performances.
...
Active les [règles recommandées](/fr/linter/rules) pour tous les groupes.
...
Active ou désactive toutes les [règles](/fr/linter/rules) pour tous les groupes.
