...
Si vous êtes déjà familiarisé avec la configuration, il se peut que vous vouliez jeter un coup d’œil à la [référence de la configuration](/fr/reference/configuration/), qui détaille toutes les options disponibles.

Biome vous permet de personnaliser son comportement en utilisant des options en ligne de commande ou un fichier de configuration appelé `biome.json` ou `biome.jsonc`.
Nous vous recommandons de créer un fichier de configuration pour chaque projet,
ce qui assure que chaque membre de l’équipe a la même configuration dans l’interface en ligne de commande et dans tout éditeur qui permet l’intégration de Biome.
Beaucoup des options disponibles dans un fichier de configuration le sont également en ligne de commande.

## Structure du fichier de configuration

Un fichier de configuration de Biome est appelé `biome.json` ou `biome.jsonc`.
Il est habituellement placé dans le répertoire racine de votre projet, à côté du `package.json` de ce dernier.

Parce que Biome est un ensemble d’outils, sa configuration est organisée autour des outils qu’il fournit.
En ce moment, Biome fournit trois outils&nbsp;: l‘outil de formatage, l’outil de linting et le trieur d’imports (aussi appelé l’organisateur d’imports).
Tous ces outils sont activés par défaut.
Vous pouvez désactiver un ou plusieurs d’entre eux en utilisant le champ `<outil>.enabled`&nbsp;:
...
Les options d’un outil propres à un langage sont placées sous un champ `<langage>.<outil>`,
...
Biome utilise l’autodécouverte pour trouver le fichier de configuration le plus proche.
Il regarde dans le répertoire de travail et dans les répertoires parents jusqu’à ce qu’il trouve un fichier `biome.json` ou `biome.jsonc`.
Si aucune configuration n’est trouvée, alors il applique les réglages par défaut de Biome.
Si `biome.json` et `biome.jsonc` sont présents en même temps dans le même dossier, priorité sera donnée à `biome.json`.
...
:::caution
Biome ne prend pas en charge les fichiers `biome.json` imbriqués, ni en ligne de commande ni en LSP. [Suivre et aider à la prise en charge dans le problème correspondant](https://github.com/biomejs/biome/issues/2228)
:::
...
ce qui vous permet de spécifier un fichier de configuration personnalisé ou un répertoire où Biome peut chercher un fichier `biome.json` ou `biome.jsonc`.
...
Elles sont traitées dans leur ordre d’apparition, les réglages dans les derniers fichiers écrasant ceux dans les premiers.
...
Pour ce faire, la première chose à faire est de mettre en place votre configuration de Biome «&nbsp;partagée&nbsp;» d’une certaine manière.
...
Biome tentera de **résoudre** votre librairie `@org/shared-configs/` depuis votre répertoire de travail.
...
:::caution
Pour éviter un changement radical avec la manière dont la résolution existante fonctionne, les chemins commençant avec un point `.` ou finissant en `.json` ou `.jsonc` **ne seront pas** résolus depuis `node_modules/`.
:::
...
Voyez la [section correspondante](#glob-syntax-explained) pour connaître les syntaxes glob prises en charge.
`include` s’applique toujours en premier avant de faire appliquer `ignore`,
...
ce qui veut dire que si vous voulez empêcher Biome de parcourir un dossier, vous devez ajouter le dossier à `ignore`.
:::
