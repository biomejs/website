...
См. [поддержка языков](/internals/language-support) для получения дополнительной информации.
...
Если вы хотите применять [небезопасные исправления](/linter/#unsafe-fixes) при сохранении, вы должны сделать [исправление кода правила безопасным](/linter/#configure-the-code-fix).
...
Не рекомендуется добавлять `biome` в ключи верхнего уровня `language_servers`, `formatter` или `code_actions_on_format` в вашем settings.json Zed. Указание biome как `language_server` или `formatter` глобально может нарушить функциональность для языков, которые biome не поддерживает (Rust, Python и т.д.). См. [поддержка языков](/internals/language-support) для полного списка поддерживаемых языков.
