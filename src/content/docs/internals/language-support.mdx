...
If you **lint** `.svelte`, `.astro` or `.vue` files and `html.expererimentalFullSupportEnabled` is `false`, it's advised to turn off a few additional rules to prevent false positive linting errors caused by our partial support. Use the option `overrides` for that:
