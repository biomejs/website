...
They disable diagnostics emitted by [linter plugins](/linter/plugins). Use `lint/plugin` to suppress diagnostics from all plugins, or `lint/plugin/<plugin-name>` to suppress diagnostics from one plugin. The `<plugin-name>` is the plugin's file name without the `.grit` extension.
