...
虽然项目和团队致力于减少破坏性更改的数量, 但 v2 仍有一些值得解释的破坏性更改. 本节将介绍最相关的破坏性更改, 解释这些破坏性更改的原因, 并提供适用的解决方案.
...
在 **Biome v1** 中, 如果运行 `npm run check`, 以下操作将会发生:
...
在 **Biome v2** 中, 如果运行 `npm run check`, 以下操作将会发生:
- Biome 在 `ui/` 中查找 `biome.json`
- 没有找到配置文件, Biome 开始检查父文件夹
- Biome 在 **父文件夹** 找到了 `biome.json`
...
虽然这与 Biome v2 没有直接关系, 但新版 VS 代码扩展使用不同的方法连接到 Biome 守护进程. 如果升级扩展, 如果使用 `"source.fixAll.biome": "explicit"`.
