...
如果你使用的是一款兼容 LSP 的 IDE，你可以配置 Biome 在保存文件时自动执行特定的辅助操作。每个辅助操作都有一个对应的"代码操作"(code action)。大多数操作遵循相同的命名模式，但也有例外情况（例如 `organizeImports`），因此请参阅每个操作的文档页面来了解相关的代码。
...
默认情况下，Biome 在运行 `check` 命令时会强制执行辅助操作。如果您 _不想强制执行辅助操作_，可以使用 `--enforce-assist` CLI 标志将其设置为 `false`。这样，即使某些操作尚未应用，Biome 也不会产生诊断错误：
