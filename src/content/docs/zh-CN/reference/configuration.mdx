...
一个指向其他Biome配置文件的路径列表。Biome解析并应用 `extends` 列表中包含的文件的配置设置，最终应用此 `biome.json`/`biome.jsonc` 文件中包含的选项。
...
你应该只在Biome需要很长时间来lint/检查你的项目的情况下，才使用此选项作为最后的手段。不支持（Glob）路径，只匹配基本名称。
