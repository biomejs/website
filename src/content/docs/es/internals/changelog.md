...
description: El registro de cambios de Biome
tableOfContents:
    maxHeadingLevel: 2
---
...
Este proyecto sigue [Semantic Versioning](https://semver.org/spec/v2.0.0.html). Debido a la naturaleza de Biome como cadena de herramientas, puede no estar claro qué cambios se consideran mayores, menores o parches. Lee nuestras [directrices para clasificar un cambio](https://biomejs.dev/internals/versioning).

Las nuevas entradas deben colocarse en una sección titulada
`Unreleased`. Lee nuestras [directrices para redactar un buen registro de cambios](https://github.com/biomejs/biome/blob/main/CONTRIBUTING.md#changelog).
...
- Fix [#4228](https://github.com/biomejs/biome/issues/4228), where the rule
  `a11y/noInteractiveElementToNoninteractiveRole` incorrectlly reports a
...
- The CSS parser now accepts more emoji in identifiers ([#3627](https://github.com/biomejs/biome/issues/3627#issuecomment-2392388022)).
  ...
  Browsers accept more emoji than the standard allows. Biome now accepts these additional emoji.
...
- The [
  `noUnmatchableAnbSelector`](https://biomejs.dev/linter/rules/no-unmatchable-anb-selector/) rule is now able to catch unmatchable
  `an+b` selectors like `0n+0` or `-0n+0`. Contributed by @Sec-ant.
- The [
  `useHookAtTopLevel`](https://biomejs.dev/linter/rules/use-hook-at-top-level/) rule now recognizes properties named as hooks like
...
- Fix a false positive in the [
  `useLiteralKeys`](https://biomejs.dev/linter/rules/use-literal-keys/) rule. ([#3160](https://github.com/biomejs/biome/issues/3160))
...
- The [stdin-file-path](https://biomejs.dev/guides/editors/first-party-extensions/#use-stdin) option now works correctly for Astro/Svelte/Vue files ([#2686](https://github.com/biomejs/biome/pull/2686))
...
- Added new `--staged` flag to the `check`, `format` and `lint` subcommands.
  ...
  _only_ to the files that are staged (the ones that will be committed), which can be very useful to simplify writing git hook scripts such as
...
- Implement [#2043](https://github.com/biomejs/biome/issues/2043): The React rule [
  `useExhaustiveDependencies`](https://biomejs.dev/linter/rules/use-exhaustive-dependencies/) is now also compatible with Preact hooks imported from
...
- Now the file name printed in the diagnostics is clickable. If you run the CLI from your editor, you can <kbd>
  Ctrl</kbd>/<kbd title="Cmd">⌘</kbd> + Click on the file name, and the editor will open said file. If row and columns are specified e.g.
...
- Fix [#1247](https://github.com/biomejs/biome/issues/1247), Biome now prints a **warning
  ** diagnostic if it encounters files that can't handle. Contributed by @ematipico
  ...
  `files.ignoreUnknown`](https://biomejs.dev/reference/configuration/#filesignoreunknown) configuration in `biome.json`:
...
- Fix [#691](https://github.com/biomejs/biome/issues/691) and [#1190](https://github.com/biomejs/biome/issues/1190), by correctly apply the configuration when computing [
  `overrides` configuration](https://biomejs.dev/reference/configuration/#overrides). Contributed by @ematipico
...
- Add [useNodejsImportProtocol](https://biomejs.dev/linter/rules/use-nodejs-import-protocol) that enforces the use of the
  `node:` protocol when importing _Node.js_ modules. Contributed by @2-NOW, @vasucp1207, and @Conaclos
...
- Add the configuration [
  `formatter.lineFeed`](https://biomejs.dev/reference/configuration/#formatterlineending). It allows changing the type of line endings. Contributed by @SuperchupuDev

- Add the configuration [
  `javascript.formatter.bracketSameLine`](https://biomejs.dev/reference/configuration/#formatterbracketsameline). It allows controlling whether ending
  `>` of a multi-line
...
- Add the configuration [
  `javascript.formatter.bracketSpacing`](https://biomejs.dev/reference/configuration/#formatterbracketspacing). It allows controlling whether spaces are inserted around the brackets of object literals. [#627](https://github.com/biomejs/biome/issues/627). Contributed by @faultyserver
