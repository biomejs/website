...
BiomeのGritQLエンジンは、独自のパーサーの上に構築されています。これにより、Biomeのネイティブな抽象構文木（AST）ノードに直接マッチするパターンを書くことができ、一般的なTreeSitter互換のパターンよりも正確な代替手段を提供します。

BiomeのネイティブASTノードは、`PascalCase`の名前（例: `JsIfStatement`）で識別され、TreeSitter互換の`snake_case`パターン（例: `if_statement`）と区別されます。

例えば、JavaScriptの `if` 文にマッチさせるには、どちらのパターンも使用できます：
...
ネイティブASTノードを使用すると、具体的で高性能なクエリが必要な場合に強力です。例えば、Biomeが認識する一部の言語機能は、一般的な `snake_case` パターンに直接の相当物がない場合があります。ASTノードに直接マッチさせることで、意図した通りの構造を正確にターゲットにすることができます。

各言語で利用可能なASTノードの完全なリストは、Biomeリポジトリの[`xtask/codegen`](https://github.com/biomejs/biome/tree/main/xtask/codegen)ディレクトリにある`.ungram`ファイルを調べることで見つけることができます。
