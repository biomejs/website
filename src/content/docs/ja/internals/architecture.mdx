...
- [`vcs.useIgnoreFile`](/ja/reference/configuration/#vcsuseignorefile) 設定が有効な場合、ネストされた `.gitignore` ファイルを検出する
- [`project` ドメイン](/linter/domains/#project) のルールが有効な場合、 `package.json` マニフェストとプロジェクト内のソースファイルをインデックス化する
...
スキャナーは [`files.includes`](/ja/reference/configuration/#スキャナーとの相互作用) 設定を通じて設定できます。
