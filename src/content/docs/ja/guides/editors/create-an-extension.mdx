...
Biome Language Serverは以下の設定を公開しており、拡張機能はこれらをユーザーに公開できます。
...
Biome Language Serverが設定ファイルを必要とするかどうか。`true`に設定すると、プロジェクトのルートに`biome.json`ファイルがあるまで、（パース以外の）ファイルの解析を行いません。
...
パスは相対パスまたは絶対パスで指定できます。Biome Language Serverは、この設定が指定された場合にのみ読み取ります。設定が*プロジェクトのサブフォルダにある*場合にこの設定を使用してください。
