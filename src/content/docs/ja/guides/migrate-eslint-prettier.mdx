...
BiomeはCSSファイルのリントとフォーマットが可能です。
多くのBiomeのCSSリンタのルールは[Stylelint](https://stylelint.io/)のルールからインスパイアされているか、同一のものです。
しかし、Biomeのルールの名前には独自の慣習があり、例えばBiomeは`camelCaseRuleName`を使う一方で、Stylelintは`kebab-case-rule-name`を使います。
Biomeは、異なるルール名を採用しているケースが多くあります。
あるルールの出所は、そのルールを説明するページで見つけることができます。
もしくは、あるStylelintのルールに対応するBiomeのルールは[Rules sources](/linter/rules-sources/)で見つけることができます。

移行を容易にするために、Biomeは`biome migrate stylelint`サブコマンドを提供します。
このサブコマンドはStylelintの設定を読み込んで、Biomeの設定に移行することを試みます。
`extends`フィールドをサポートしていて、プラグインの設定と共有設定の両方を読み込みます。
このサブコマンドはStylelintの設定ファイルに設定された全てのプラグインと`extends`フィールドを読み込むためにNode.jsを必要とします。
また、このサブコマンドは`ignoreFiles`フィールドも移行します。
...
Stylelintの設定をBiomeに移行するために以下のコマンドを実行できます。
...
現在は、`biome migrate stylelint`はJSON（`.stylelintrc.json`）とJavaScript（`.stylelintrc.js`、`.stylelintrc.cjs`、`.stylelintrc.mjs`）で書かれた設定ファイルのみをサポートしています。
YAMLで書かれた設定ファイルはサポートしていません。
...
Biomeがいくつかのルールオプションを実装しないか、元の実装からわずかに逸脱することを選択したため、Stylelintとまったく同じ動作を得る可能性は低いことに注意してください。

StylelintはVCS ignoreファイルを使用しているので、Biomeの[VCS インテグレーション](/ja/guides/integrate-in-vcs/)を有効にすることを推奨します。
