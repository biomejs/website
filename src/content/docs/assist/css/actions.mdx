...
| --- | --- | --- |
| [useSortedProperties](/assist/actions/use-sorted-properties) | Enforce ordering of CSS properties and nested rules. | <span class='inline-icon' title='The rule has a safe fix.'><Icon name="seti:config" label="The rule has a safe fix" size="1.2rem"  /></span><span class='inline-icon' title="This rule is only available in beta"><Icon name="moon" label="This rule is only available in beta" size="1.2rem" /></span> |
## Recommended rules 
