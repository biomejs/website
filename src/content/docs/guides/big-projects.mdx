...
shared file itself. Plugin paths normally follow that same rule too. The
exception is when a shared configuration needs to load a plugin relative to the
file that declared it. In that case, use the object syntax and set
