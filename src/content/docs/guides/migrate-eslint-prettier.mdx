...
**Workaround:** If the migration fails with a cyclic reference error, temporarily comment out
the plugins or shared configurations that are causing the issue, run the migration, and then
re-enable them one by one to identify the problematic configuration.

For example, if your `eslint.config.js` looks like this:
...
Try commenting out the problematic entries and re-running the migration:
