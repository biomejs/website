...
For example, the Zed extension maps the [`require_configuration`](#require-configuration) from the setting is called `require_config_file`, which is defined in a JSONC file.
:::
...
Whether the Biome Language Server requires a configuration file. When set to `true`, it won't analyze any file.
...
The path *should* be an absolute. The Biome Language Server read this option only when provided.

```json
{
  "configuration_path": "/Users/nick/.configs/biome.json"
}
```
