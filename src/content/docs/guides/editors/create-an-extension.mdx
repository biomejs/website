...
Whether the Biome Language Server requires a configuration file. When set to `true`, it won't analyze any file (exception for parsing) until there's a `biome.json` file in the root of the project.
