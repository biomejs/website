...
<script>
	// The homepage overrides in `src/styles/_overrides.css` give these two code
	// blocks a fixed height and `overflow: scroll`, which moves the scroll
	// container from Expressive Code's `<pre>` onto the `<code>` inside it.
	// Neither element is focusable, so a keyboard user cannot scroll them and the
	// lines below the fold are unreachable. A mouse user sees all of it.
	//
	// WCAG 2.1.1 Keyboard (Level A); axe rule `scrollable-region-focusable`.
	for (const code of document.querySelectorAll(".linter pre > code, .format pre > code")) {
		if (
