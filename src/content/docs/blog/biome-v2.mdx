...
Biome is now able to support nested configuration files out of the box—CLI and editors, whether they are inside a monorepo, or in another different setting.

Until now nested configuration files were *somewhat* supported in the CLI; however, the developer experience inside editors was essentially none.

Nested configuration files must have a new configuration field `"root"` set to **`false`**. If your project already uses nested configuration files,
you should run the command `biome migrate` **from the root of the project**, and all your configuration files will be updated accordingly.

With monorepo support, you have a new tool at your disposal, which is new microsyntax `"extends": "//"`, only available to nested configuration files.
This syntax means "`extends` from the root configuration". Say goodbye to wonky relative paths such as `"extends": ["../../biome.json"]` 👋
...
No software is exempt from bugs, so we will make sure to squish them and release patches.
...
I want to remind you that Biome is a project lead by volunteers that like programming, open-source, and embrace of the Biome philosophy, so any help is welcome 😁

#### Translations

If you know Biome and you want to help the outreach of the tool, you can help us by translating the website to your native language. In this [dashboard](https://biomejs.dev/i18n-dashboard/) you can check the supported languages and if they are up to date.

#### Chat with us

Join our [Discord server](https://biomejs.dev/chat), and engage with the community. We believe that chatting with the community and being part of the community as a form of contribution.

#### Code contributions

If you like the technical aspects project, and you want to make your way into Rust language, or practice more your knowledge around parsers, compilers, analyzers, etc., Biome is the project that does for you!

There are a lot of aspects that you can explore; I assure you that you won't get bored. Here are a small list of the things you can start with:
- Create new lint rules! We have some many rules that we haven't implemented yet (ESLint, ESLint plugins, Next.js, Solid, etc.). We have a very [extensive technical guide](https://github.com/biomejs/biome/blob/main/crates/biome_analyze/CONTRIBUTING.md).
- [Help](https://github.com/biomejs/biome/blob/main/crates/biome_parser/CONTRIBUTING.md) [building](https://github.com/biomejs/biome/tree/main/crates/biome_yaml_parser) [Biome](https://github.com/biomejs/biome/tree/main/crates/biome_html_parser) [parsers](https://github.com/biomejs/biome/tree/main/crates/biome_markdown_parser)!
  One interesting fact about Biome parsers is that they are recoverable parsers [error resilient](/internals/architecture/#parser-and-cst) which emits a [CST](https://en.wikipedia.org/wiki/Parse_tree) instead of a classic AST.
