...
Among all the features shipped in this release, here's the ones we think you're going to like most!
...
These rules work seamlessly with Vue, Svelte, and Astro files.

### Rule profiler

The commands `lint` and `check` has now a `--profile-rules` flag. This flag enables the new internal profiler, which allows you to capture the execution time of lint rules, assist actions, and GritQL plugins.
...
A way to interpreter the data is to check the rules/actions that have low counts, and check their execution time. If the execution time feels off, for example, way too high compared to what it should be, maybe it's a good place to look for possible optimizations. Since we landed this feature, we found some bottlenecks that we fixed since then.
...
- [`noShadow`](https://biomejs.dev/linter/rules/no-shadow/) - Reports variable shadowing
...
Added support for parsing `:global` and `:local` inside `.astro`, `.svelte` and `.vue` files, in the `<style>` portion of the file. This capability is only available when `experimentalFullHtmlSupportedEnabled` is set to `true`.
