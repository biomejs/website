...
- **`    --apply`** &mdash; 
  Apply safe fixes, formatting and import sorting	  Alias for `--write`, writes safe fixes (deprecated, use `--write`)
- **`    --apply-unsafe`** &mdash; 
  Apply safe fixes and unsafe fixes, formatting and import sorting	  Alias for `--write --unsafe`, writes safe and unsafe fixes (deprecated, use `--write --unsafe`)
- **`    --stdin-file-path`**=_`PATH`_ &mdash; 
  Use this option when you want to format code piped from `stdin`, and print the output to `stdout`.
