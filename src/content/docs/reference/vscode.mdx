...
You should be aware of that `@biomejs/biome` doesn't ship any binaries. The fine `@biomejs/biome/bin` is just a tiny
wrapper that delegates the the real binary. The binary installed on your machine depends on the architecture of your OS and architecture.
